...
<?php
    ...
    foreach ($backups as $db_field) {
        ...
        echo "<tr valign='middle'><td>" . $date . "</td><td><center>" . $version . "</td><td><a href='" . $filename . "'>DOWNLOAD</a></td><td><center><form action='listbackups.php' method='POST'><input type='hidden' name='delbackup' value='delbackup'><input type='hidden' name='backupid' value='" . $backupid . "'><input type='hidden' name='id' value='" . $id . "'><input type='hidden' name='name' value='" . $name . "'><input type='submit' value='Delete' onclick='return window.confirm(\"Are you sure you want to delete " . $filename . "\");'class='btn-xs btn-danger'></form></td></tr>";
    }
